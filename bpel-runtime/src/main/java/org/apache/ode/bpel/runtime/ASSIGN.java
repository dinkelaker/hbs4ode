...
class ASSIGN extends ACTIVITY {
     ...
     *             DOCUMENTME
     */
    // AL: pass OAssing.Copy instead of OAssing.Copy.RVALUE 
    private Node evalRValue(OAssign.Copy ocopy) throws FaultException, ExternalVariableModuleException {
    	OAssign.RValue from = ocopy.from;
        ...
        if (from instanceof DirectRef) {
            ...
            VariableInstance vinst = _scopeFrame.resolve(dref.variable);
            // AL: GetVar
    		try {
    			history.getVar(getBpelRuntimeContext().getPid(),
    					new ODEReadVarialeFact(ocopy, vinst),
            ...
            VariableInstance vinst = _scopeFrame.resolve(varRef.variable);
            // AL: GetVar
    		try {
    			history.getVar(getBpelRuntimeContext().getPid(),
    					new ODEReadVarialeFact(ocopy, vinst),
            ...
            VariableInstance vinst = _scopeFrame.resolve(propRef.variable);
            // AL: GetVar
    		try {
    			history.getVar(getBpelRuntimeContext().getPid(),
    					new ODEReadVarialeFact(ocopy, vinst),
	...
	private void copy(OAssign.Copy ocopy) throws FaultException, ExternalVariableModuleException {
        ...
        if ((ocopy.to instanceof VariableRef && ((VariableRef) ocopy.to)
        	...
        	// AL: pass ocopy instead of ocopy.to / ocopy.from
            Node rvalue = evalRValue(ocopy);
            ...
            if (ocopy.to instanceof OAssign.PartnerLinkRef) {
                ...
                commitChanges(ocopy, lval, lvalue);
                se = new VariableModificationEvent(lval.declaration.name);
                ((VariableModificationEvent)se).setNewValue(lvalue);                
            }
        }
