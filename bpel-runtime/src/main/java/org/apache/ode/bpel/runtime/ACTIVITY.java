...
abstract class ACTIVITY extends BpelJacobRunnable implements IndexedObject {
	...
	protected static final BpelHistory history = BpelHistory.getInstance();
    ...
    // AL: added source parameter
    void commitChanges(OBase source, VariableInstance lval, Node lvalue) 
        throws ExternalVariableModuleException
    {
		...
		try {
			history.setVar(getBpelRuntimeContext().getPid(),
					new ODEWriteVariableFact(source, lval, lvalue),
