...
import org.apache.ode.bpel.runtime.history.ODEBpelProcess;
...
import tud.st.bpel.history.NullPolicyViolationHandler;
...
class BpelRuntimeContextImpl implements BpelRuntimeContext {
    ...
    private final BpelHistory history = BpelHistory.getInstance();
    ...
    public void completedFault(FaultData faultData) {
   	 	...
   	 	// AL: process done fault    	
    	try {
			history.processEnded(this.getPid(),
					new ODEBpelProcess(_bpelProcess.getOProcess()),
					new NullPolicyViolationHandler());
    ...
    public void completedOk() {
    	...
    	try {
			...
			__log.error(e.getMessage());
			FaultData fault = new FaultData(new QName("PolicyViolatedException"),
					_bpelProcess.getOProcess(),
					e.getMessage());
    ...
    public Node writeVariable(VariableInstance variable, Node changes) {
    	...
    	//AL: SetVar TODO: remove
    	// Don't execute if a policy is violated
    	/* Now done using the VariableModificationEvent
    	try {
    		history.setVar(source, variable.declaration.name, DOMUtils.domToString(changes));
    	}
    	catch(PolicyViolatedException e) {
    		__log.error(e);
            return null;
