...
import org.apache.ode.bpel.dd.PoliciesType;
import org.apache.ode.bpel.dd.PolicyType;
...
public class ProcessStoreImpl implements ProcessStore {
    ...
    public Collection<QName> deploy(final File deploymentUnitDirectory) {
        ...
        // AL: Add Policies
        TDeployment tDep = dd.getDeploy();
        PoliciesType polType = tDep.getPolicies();
        if(polType != null) {
        	if(polType.isSetAllowFaultHandlers()) {
        		history.setAllowFaultHandlers(polType.getAllowFaultHandlers());
        	}
        	else {
        		history.setAllowFaultHandlers(false);
        	}
        	List<PolicyType> policies = polType.getPolicyList();
        	if(policies != null) {
        		System.out.println("Found " + policies.size() + " policies!");        		
        		for(PolicyType p : policies) {
        			history.addPolicy(p.getName(), p.getFaultName(), p.newCursor().getTextValue());
        		}
        	}
        	else {
        	...
        	System.out.println("deploy.xml does not contain policies");
        }
